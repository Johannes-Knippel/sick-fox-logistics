...
import hm.edu.tracking.senseit.entity.SigfoxMessage;
...
public class SensitApi {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{name}")
    public Response getMessagesForName(@PathParam("name") String name) {
        log.info("SensitApi: GET request on /api/senseit");
        List<SenseitMessage> senseitMessages = service.getAllBooks();
    ...
    public Response addMessage(SenseitMessage newSenseitMessage,
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("sigfox/batch")
    public Response addSigfoxMessages(SigfoxMessage[] messages) {
        for (SigfoxMessage mes : messages) {
            SenseitMessage newMes = mes.toSenseitMessage();
            service.createOrUpdate(newMes);
        }
    ...
    @Path("sigfox")
    public Response addSigfoxMessages(SigfoxMessage messages) {
        SenseitMessage newMes = messages.toSenseitMessage();
        service.createOrUpdate(newMes);
        return Response.ok().build();
    }
    ...
    @DELETE
    @Path("/{id:[0-9]*}")
    public Response deleteMessagePerId(@PathParam("id") long id) {
